...
int coursesleft = chosenCourses.size();
if(k <=  coursesleft){
    for (int i = 0; i < k; i++)
    {
        cout << chosenCourses[i].name;
        for (int j = 0; j < chosenCourses[i].sessions.size(); j++)
        {
        cout << chosenCourses[i].sessions[j].day;
        cout << chosenCourses[i].sessions[j].startTime;
        cout << chosenCourses[i].sessions[j].endTime;
        }
